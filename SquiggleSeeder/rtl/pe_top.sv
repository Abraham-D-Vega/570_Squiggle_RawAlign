// Top level for MAX_NUM_PEs PEs per segment
`include "utils.svh"
`timescale 1ns/1ps
module pe_top (
    input logic clk,
    ...
    logic  [`NUM_PEs-1:0][31:0] prev_scores;
    Anchor [`NUM_PEs-1:0]       prev_starts;
    Anchor [`NUM_PEs-1:0]       prev_ends;
    logic [`NUM_PEs-2:0][31:0]        scores_back;
    // PE passing variables
    logic [`NUM_PEs-1:0] valid_out;    
    ...
    pe_start pe_start_inst (
        ...
        .prev_starts(prev_starts[`NUM_PEs-1:1]), 
        .prev_ends(prev_ends[`NUM_PEs-1:1]), 
    ...
    // Generate MAX_NUM_PEs-1 PEs here
    genvar i;
    generate
        for(i = 0; i < `NUM_PEs-1; i++) begin : pe_instances
            pe pe_inst(
                .clk(clk),
    ...
    always_comb begin
       ...
       // $display("what the sigma valid_out = ", valid_out[`NUM_PEs-1]);
        if(valid_out[`NUM_PEs-1] == 1'b1 && (prev_scores[`NUM_PEs-1] > best_score_r)) begin
            n_best_end = prev_ends[`NUM_PEs-1];
            n_best_start = prev_starts[`NUM_PEs-1];
            n_best_score = prev_scores[`NUM_PEs-1];
        end
        done = ~valid_out[`NUM_PEs-1] & (best_score_r != 0);
    end
